...
跑 `python -m gui.server` 的终端会在 AI 落子、点击「提示」、对局结束时实时打印一行胜率判断（红方视角百分比 + 原始 value + 耗时 + 备选着法），无需打开浏览器控制台即可跟踪模型判断。格式示例：
```
[14:32:07] [对局 3f2a | 第12手] AI落子 炮二平五 | 红方胜率 63.2% (value=+0.264) | sims 400 | 耗时 1.8s | 备选: 马八进七 21% 车九平八 11%
```
